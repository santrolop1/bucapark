    ...
    <div className="min-h-screen bg-[#f4f7f0] text-[#1a332a]">
      <div className="bg-white/90 backdrop-blur sticky top-0 z-30 border-b border-[#e0e8db]">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <h1 className="text-xl font-black">Mis reservas</h1>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="mb-6">
          <p className="text-sm text-[#5a6b5f]">
            Aquí ves tus reservas registradas en el sistema. Solo se muestran las reservas de tu usuario autenticado.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-[#e0e8db] p-8 text-center">
            ...
            <h2 className="text-xl font-black mb-2">No hay reservas</h2>
            <p className="text-sm text-[#5a6b5f]">Aún no registraste ninguna reserva.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {reservations.map((reservation) => (
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div>
                    <h2 className="text-lg font-bold text-[#1a332a]">{reservation.proposito || reservation.tipoEspacio || 'Reserva'}</h2>
                    <p className="text-sm text-[#5a6b5f]">{reservation.tipoEspacio || reservation.espacio || 'Espacio'} • {reservation.personas || 1} personas</p>
                  </div>
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  <div className="rounded-2xl bg-[#f8faf6] p-3">
                    ...
                    <p className="mt-2 font-bold text-[#1a332a]">{formatDate(reservation.fecha)}</p>
                  </div>
                  <div className="rounded-2xl bg-[#f8faf6] p-3">
                    <p className="text-xs uppercase tracking-[0.2em] text-[#5a6b5f]">Hora</p>
                    <p className="mt-2 font-bold text-[#1a332a]">{reservation.horaInicio || '—'}</p>
                  </div>
                  <div className="rounded-2xl bg-[#f8faf6] p-3">
                    <p className="text-xs uppercase tracking-[0.2em] text-[#5a6b5f]">Parque</p>
                    <p className="mt-2 flex items-center gap-2 text-[#1a332a]">
                      <MapPin className="h-4 w-4" />
                      {reservation.parkId || 'Parque desconocido'}
                    </p>
                  </div>
                </div>
              </motion.div>

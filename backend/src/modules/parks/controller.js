...
exports.getParkById = async (req, res) => {
  try {
    ...
    if (!park) return res.status(404).json({ success: false, error: 'Parque no encontrado' });
...
exports.updatePark = async (req, res) => {
  try {
    const park = await Park.findByIdAndUpdate(req.params.id, pickAllowed(req.body), { new: true, runValidators: true });
    if (!park) return res.status(404).json({ success: false, error: 'Parque no encontrado' });
